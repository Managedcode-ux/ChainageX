...
from app.controller.diesel_controller import create_diesel_issued_entry, get_diesel_issued_entry, \
    get_all_diesel_issued_entries, delete_diesel_issued_entry,update_diesel_issued_entry
...
async def update_diesel_entry(data: RequestSchema_DieselIssued_Update, recordId: str, db: Session = Depends(get_db)):
    updated_data = await update_diesel_issued_entry(recordId, db, data)
    response_data = ResponseSchema_DieselIssued_Create.model_validate(updated_data)
    return APIResponse(
